...
> **Note**: ES6モジュールを使用しているため、ローカルファイルでは一部ブラウザで動作しない場合があります。開発サーバー経由での起動を推奨します。
...
npm run build   # ビルド（lint + format）
npm run validate # 全体検証（build + test）
```
