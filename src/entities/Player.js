...
class Player {
    ...
    update(dt, keys, gamepad, canvas, bullets, particles, audioSystem) {
        ...
        if (gamepad) {
            const ax = gamepad.axes[2], ay = gamepad.axes[3];
        ...
        if ((sdir.x || sdir.y) && this.cd <= 0 && bullets) {
            ...
            if (this.powerups.triple > 0) {
                ...
                for (let i = -1; i <= 1; i++) {
                    ...
                    bullets.push(new Bullet(shotOrigin, bulletDir.mul(this.gameConfig.bulletSpeed), this.gameConfig));
                }
            } else if (this.powerups.laser > 0) {
                // Laser shot (faster, piercing bullets)
                bullets.push(new LaserBullet(shotOrigin, shotDir.mul(this.gameConfig.bulletSpeed * 1.5), this.gameConfig));
            } else {
                // Normal shot
                bullets.push(new Bullet(shotOrigin, shotDir.mul(this.gameConfig.bulletSpeed), this.gameConfig));
            }
