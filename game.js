...
function spawnBurst(pos, color, count = GameConfig.particleCount, type = 'circle') {
  for (let i = 0; i < count; i++) {
    ...
    const speed =
      ...
      Math.random() * (GameConfig.particleMaxSpeed - GameConfig.particleMinSpeed);
...
class Enemy {
  ...
  update(dt, playerPos) {
    ...
    if (this.type === 'homing' && playerPos) {
      ...
      const targetVel = toPlayer.mul(
        Math.min(currentSpeed + this.homingStrength * dt, this.maxSpeed)
      );
    ...
    if (this.type === 'splitter') {
      ...
      ctx.fillStyle =
        this.color
          .replace(')', `, ${alpha})`)
          .replace('#', 'rgba(')
          .replace(/(.{2})/g, (match, p1) => parseInt(p1, 16) + ',')
          .slice(0, -1) + ')';
      ctx.shadowBlur = 0;
      ctx.beginPath();
      ctx.arc(this.pos.x, this.pos.y, this.r * 0.6, 0, Math.PI * 2);
...
// ===== Game State =====
let player, bullets, enemies, particles, powerups, lastTs;
...
function updateUI() {
  ...
  document.getElementById('score').textContent = uiData.score;
  document.getElementById('lives').textContent = uiData.lives;
  document.getElementById('bombs').textContent = uiData.bombs;
  document.getElementById('highscore').textContent = uiData.highScore;

  // Update combo display
  const comboDisplay = document.getElementById('combo-display');
  if (uiData.comboCount > 3) {
    comboDisplay.style.display = 'block';
    document.getElementById('combo').textContent = uiData.comboCount;
    document.getElementById('multiplier').textContent = uiData.scoreMultiplier.toFixed(1);
  } else {
...
function spawnEnemy() {
  ...
  let pos, vel;
  const speedMult = gameState.getDifficultyMultiplier();
  ...
  switch (edge) {
    case 0: // Top
      ...
      vel = new Vec2(
        ...
        baseSpeed * 0.5 + Math.random() * baseSpeed * 0.5
      );
      break;
    case 1: // Right
...
function useBomb() {
  if (!gameState.useBomb() || bombEffect) return false;

  updateUI();

  // Create bomb effect
  bombEffect = new BombEffect();
  gameState.setBombEffect(bombEffect);
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    ...
    if (distance <= bombEffect.maxRadius) {
      ...
      gameState.addScore(destroyedEnemy.scoreValue);
      destroyedCount++;
  ...
  if (destroyedCount > 5) {
    gameState.addScore(destroyedCount * 5); // Bonus points
  }
...
function gameOverScreen() {
  ...
  const uiData = gameState.getUIData();
  renderingSystem.renderer.renderGameOverScreen(uiData.score, uiData.highScore, isNewHighScore);
}
...
function update(dt) {
  if (!gameState.isPlaying()) return !gameState.isGameOver();
  ...
  outer: for (let i = enemies.length - 1; i >= 0; i--) {
    ...
    for (let j = bullets.length - 1; j >= 0; j--) {
      ...
      if (e.pos.sub(b.pos).len() < e.r + b.r) {
        ...
        const points = gameState.addScore(destroyedEnemy.scoreValue);
        updateUI();
        spawnExplosion(destroyedEnemy.pos, destroyedEnemy.color);
        ...
        if (Math.random() < GameConfig.powerupDropChance) {
          if (Math.random() < 0.2 && gameState.addBomb()) {
  ...
  if (gameState.updateSpawnTimer(dt)) {
    spawnEnemy();
  }
...
function draw() {
  ...
  // Draw UI elements
  renderer.renderUI(gameState.getState(), uiData);
...
window.addEventListener('keydown', e => {
  if (e.code === 'KeyR' && gameState.isGameOver()) {
    reset();
  } else if (e.code === 'KeyP' && (gameState.isPlaying() || gameState.isPaused())) {
  ...
  } else if (e.code === 'Escape' && gameState.isPaused()) {
    gameState.resume();
...
function pollGamepad() {
  ...
  if (gp) {
    ...
    if (gameState.isGameOver() && gp.buttons[9].pressed && !lastGamepadButtons[9]) {
      reset();
    } else if (
      (gameState.isPlaying() || gameState.isPaused()) &&
      gp.buttons[8].pressed &&
      !lastGamepadButtons[8]
    ) {
